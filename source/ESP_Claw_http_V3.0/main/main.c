...
void app_main(void)
{
    ...
    /* Clean up FATFS files that may have been corrupted by a previous
     * ungraceful shutdown. The scheduler init function will recreate it as a
     * valid empty file. Router rules must be preserved so agent replies can be
     * delivered back to IM/WebIM channels. */
    unlink("/fatfs/scheduler/schedules.json");
