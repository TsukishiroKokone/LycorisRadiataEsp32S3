...
export type PinCapability = 'gpio' | 'adc' | 'pwm' | 'i2c' | 'onewire';
...
export type ModuleSummary = {
  id: string;
  category: 'sensors' | 'actuators' | 'media';
  implemented: boolean;
};
...
export function fetchPins(capability: PinCapability) {
  return request<PinsResponse>(
    '/api/pins?capability=' + encodeURIComponent(capability),
    undefined,
    'Failed to load pin list',
  );
}

export async function fetchModules() {
  const data = await request<{ items: ModuleSummary[] }>(
    '/api/modules',
    undefined,
...
export function fetchModuleConfig(moduleId: string) {
  return request<ModuleConfig>(
    '/api/modules/' + encodeURIComponent(moduleId) + '/config',
    undefined,
    'Failed to load module config',
  );
}

export function saveModuleConfig(moduleId: string, config: ModuleConfig) {
  return request<ModuleConfig>(
    '/api/modules/' + encodeURIComponent(moduleId) + '/config',
    {
      method: 'POST',
...
export function deleteModuleConfig(moduleId: string) {
  return request<{ ok?: boolean }>(
    '/api/modules/' + encodeURIComponent(moduleId) + '/config',
    { method: 'DELETE' },
    'Failed to clear module config',
  );
}

export function moduleAction(
  moduleId: string,
  action: 'read' | 'start' | 'stop' | 'control',
  config: ModuleConfig,
) {
  return request<ModuleActionResult>(
    '/api/modules/' + encodeURIComponent(moduleId) + '/' + action,
    {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(config),
    },
    'Failed to run module action',
  );
}
